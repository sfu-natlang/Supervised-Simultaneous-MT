...
def _main(cfg: DictConfig, output_file):
    ...
    nmt_models, nmt_saved_cfg = checkpoint_utils.load_model_ensemble(
    ...
    task.load_dataset(cfg.dataset.gen_subset, task_cfg=nmt_saved_cfg.task)
    ...
    for model in chain(nmt_models, lms):
        if model is None:
            continue
        if cfg.common.fp16:
            model.half()
    ...
    itr = task.get_batch_iterator(
        ...
        max_positions=utils.resolve_max_positions(
            task.max_positions(), *[m.max_positions() for m in nmt_models]
        ),
    ...
    generator = task.build_simultaneous_generator(
        nmt_models, cfg.generation,
