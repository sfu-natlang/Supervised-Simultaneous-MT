...
def get_parser():
    ...
    parser.add_argument(
        "-a", "--action-lang", default=None, metavar="TARGET", help="target action"
    )
    parser.add_argument(
    ...
    parser.add_argument("--actdict", metavar="FP", help="reuse given action dictionary")
    parser.add_argument(
        "--nwordstgt",
        metavar="N",
        default=-1,
...
def main(args):
    ...
    action = True if args.action_lang is not None else False
    ...
    if args.inputtype == "text":
        if args.joined_dictionary:
            ...
            if action is not None:
                ...
                act_dict = dictionary.Dictionary.load(args.actdict)
            if target:
                if args.tgtdict:
    ...
    if action:
        act_dict.save(dict_path(args.action_lang))
