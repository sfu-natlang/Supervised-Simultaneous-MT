...
def pad_fixed_sequence(seq, padding=1, seq_len=7):
    ...
    padded_sequence = seq if pad_length == 0 else torch.cat((torch.tensor([padding]*pad_length), seq))
...
def generate_incremental_net_input(hypos, num_src=7, src_pad=1, trg_pad=1):
    ...
    final_input = torch.ones(len(hypos), len(hypos[0]['action_seq'])-1, num_src+1)
    for i, hypo in enumerate(hypos):
        ...
        for j in range(1, len(hypo['action_seq'])):
            ...
            input_elements = torch.cat((src_tokens, torch.tensor([trg_tokens])))
            sub_input[j-1] = input_elements
        final_input[i][:sub_input.shape[0]] = sub_input
    return torch.tensor(final_input, dtype=torch.long)
