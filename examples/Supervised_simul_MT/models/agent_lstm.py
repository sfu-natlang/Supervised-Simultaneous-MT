...
class AgentLSTMModel(FairseqEncoderDecoderModel):
    ...
    def forward(
        ...
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, enforce_sorted=False)
...
class LSTMEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        num_embeddings_src = len(src_dictionary)
        self.padding_idx = padding_idx if padding_idx is not None else src_dictionary.pad()
        if pretrained_embed is None:
            self.src_embed_tokens = Embedding(num_embeddings_src, embed_dim, self.padding_idx)
        else:
            self.src_embed_tokens = pretrained_embed

        num_embeddings_trg = len(trg_dictionary)
        self.trg_embed_tokens = Embedding(num_embeddings_trg, embed_dim, self.padding_idx)
    ...
    def forward(
        ...
        """
        Args:
            tokens (LongTensor): tokens in the source language of
                shape `(batch, src_len)`
        ...
        bsz, seqlen, featlen = tokens.size()
        src_tokens = tokens[:, :, :-1]
        trg_tokens = tokens[:, :, -1:]

        src_tokens = torch.reshape(src_tokens, (bsz*seqlen, src_tokens.shape[-1]))
        trg_tokens = torch.reshape(trg_tokens, (bsz*seqlen, trg_tokens.shape[-1]))

        # embed tokens
        x1 = self.src_embed_tokens(src_tokens)
        x2 = self.trg_embed_tokens(trg_tokens)
        x = torch.cat((x1, x2), dim=1)
        x = torch.reshape(x, (bsz*seqlen, x.shape[1]*x.shape[2]))
        x = torch.reshape(x, (bsz, seqlen, x.shape[1]))
...
@register_model_architecture("agent_lstm", "agent_lstm_wiseman_iwslt_de_en")
def agent_lstm_wiseman_iwslt_de_en(args):
    args.dropout = getattr(args, "dropout", 0.1)
...
@register_model_architecture("agent_lstm", "agent_lstm_luong_wmt_en_de")
def agent_lstm_luong_wmt_en_de(args):
